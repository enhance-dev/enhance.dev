...
function Enhancer(options = {}) {
  ...
  return function html(strings, ...values) {
    ...
    const templateNames = Object.keys(elements).filter((element) => usedElements.includes(element));
    const templates = parseFragment(templateNames.map((name) => template({
      name,
      elements,
      store,
      scriptTransforms,
  ...
  const find = (node2) => {
    for (const child of node2.childNodes) {
      if (isCustomElement(child.tagName)) {
        ...
        const template2 = expandTemplate(child, elements, store);
        fillSlots(child, template2);
      }
      if (child.childNodes)
        find(child);
    }
...
function fillSlots(node, template2) {
  const slots = findSlots(template2);
  ...
  for (let i = 0; i < slots.length; i++) {
        ...
        for (let i3 = 0; i3 < insertsLength; i3++) {
          ...
          for (let i4 = 0; i4 < insertAttrsLength; i4++) {
            ...
            if (insertSlot === slotName) {
              ...
              slotParentChildNodes.splice(slotParentChildNodes.indexOf(slot), 1, insert);
              usedSlots.push(slot);
            }
          }
        }
    ...
    if (!hasSlotName) {
      ...
      const slotParentChildNodes = slot.parentNode.childNodes;
      slotParentChildNodes.splice(slotParentChildNodes.indexOf(slot), 1, ...children);
    }
  }
  const unusedSlots = slots.filter((slot) => !usedSlots.includes(slot));
  replaceSlots(template2, unusedSlots);
  const nodeChildNodes = node.childNodes;
  nodeChildNodes.splice(0, nodeChildNodes.length, ...template2.childNodes);
}
...
function replaceSlots(node, slots) {
  slots.forEach((slot) => {
    ...
    const slotParentChildNodes = slot.parentNode.childNodes;
    slotParentChildNodes.splice(slotParentChildNodes.indexOf(slot), 1, ...slot.childNodes);
  });
  return node;
}
...
function addScriptStripper(body) {
  const stripper = parseFragment(`<script>Array.from(document.getElementsByTagName("template")).forEach(t => t.content.lastElementChild && 'SCRIPT' === t.content.lastElementChild.nodeName?document.body.appendChild(t.content.lastElementChild):'')<\/script>`);
